...
export default function Navbar(props) {
    ...
    const [expSearch,setExpSearch] = useState(0)
            ...
            <div>
                <nav className={``}>
                    <div className={`container1 w-full transition-all duration-300 ${props.darkMode?'bg-[#202124] text-[#e1e1e2]':'bg-white text-[#5f6368]'} fixed top-0 py-2 flex`}>
                        <h1 className='font-semibold w-1/2 lg:w-1/4 text-2xl lg:ml-3 p-2'> <Link to='/'>NewsMonkey</Link></h1>
                        <div className='flex lg:space-x-5 w-1/2 items-center justify-end '>
                            <form className='flex'>
                            ...
                            <Link className={`lg:hidden transition-all duration-300 font-bold ${!expSearch?'scale-100':'scale-0'} z-50 relative right-[5px] top-1px`} to={`/${input}`}>Go</Link>
                                ...
                                <input id={searchId} value={input} onInput={e => setInput(e.target.value)} className={`hidden lg:block transition-all duration-300 ${props.darkMode?'bg-[#3d4043]':'bg-[#f1f3f4]'} pl-4 outline-none py-3 lg:w-[35rem] rounded-l-none rounded-lg`} placeholder='Search for topics, locations & sources' type="text" name="searchText" />
                                <Link className={`hidden lg:block transition-all duration-300 font-bold z-50 fixed right-[31.35rem] top-[1.15rem]`} to={`/${input}`}>Go</Link>
                            </form>
                    ...
                    <div className={`container2 transition-all duration-300 rounded-lg lg:rounded-none ${!expand?'right-[-140px]':'right-[5px]'} lg:right-0 ${props.darkMode?'bg-[#686868] lg:bg-[#202124] border-[#5e5e5e] text-[#e1e1e2]':'bg-white border-[#e3e3e3] text-[#5f6368]'} top-[70px] lg:w-full lg:top-16 lg:pt-4 fixed shadow-sm lg:border-b-[1px] lg:pb-3`}>
